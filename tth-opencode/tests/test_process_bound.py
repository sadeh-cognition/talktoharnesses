...
from tth_opencode import service, sessions
...
class SpawnFakeAdapter:
    ...
    async def probe(self, config: HarnessConfiguration) -> HarnessCapabilities:
        del config
        return HarnessCapabilities(kind=HarnessKind.OPENCODE, version="1.0.3-R2198.1")
...
def spawn_adapter(monkeypatch: pytest.MonkeyPatch) -> Iterator[SpawnFakeAdapter]:
    ...
    monkeypatch.setenv("TALKTOHARNESSES_OPENCODE_EXECUTABLE", sys.executable)
