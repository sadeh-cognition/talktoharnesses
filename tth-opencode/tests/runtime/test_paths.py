...
from tth_opencode.shared.paths import (
    resolve_directory,
    resolve_executable,
...
def test_executable_owner_mismatch(owned_python: Path) -> None:
    ...
    with (
        patch("tth_opencode.shared.paths.os.geteuid", return_value=os.geteuid() + 1),
...
def test_effective_access_fallback_and_stat_failure(tmp_path: Path, owned_python: Path) -> None:
    from tth_opencode.shared import paths as paths_mod
    ...
    with (
        patch("tth_opencode.shared.paths.sys.platform", "win32"),
        patch(
            "tth_opencode.shared.paths._windows_file_and_token_sids",
...
def test_execute_check_uses_effective_ids(owned_python: Path) -> None:
    ...
    with patch("tth_opencode.shared.paths.os.access", side_effect=access):
