...
from tth_opencode import settings
from tth_opencode.shared.logs import UtcFormatter
...
def test_split_logger_propagates_to_root_handlers() -> None:
    ...
    try:
        logging.getLogger("tth_opencode.tests.propagation").info("hello from the split")
    ...
    assert logging.getLogger("tth_opencode").propagate is True
    assert not logging.getLogger("tth_opencode").handlers
