...
from talktoharnesses.providers.compatibility import ReleaseCapabilities, enforce_operation
...
def test_known_release_helpers_resolve() -> None:
    ...
    floor = load_opencode_compatibility().floor
    assert match_opencode(floor.version, platform="linux").id == f"opencode-{floor.version}"
