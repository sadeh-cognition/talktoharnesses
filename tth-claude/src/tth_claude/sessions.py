"""In-memory session store: one adapter and one frame stream per session.
...
# pump blocks on a full queue, which backpressures the harness stream.
...
class SessionStore:
    ...
    def add(
        ...
        *,
        ...
        entry.pump_task = asyncio.get_running_loop().create_task(
            self._pump(entry),
