...
from tth_claude import settings
from tth_claude.shared.logs import UtcFormatter
...
def test_split_logger_propagates_to_root_handlers() -> None:
    ...
    try:
        logging.getLogger("tth_claude.tests.propagation").info("hello from the split")
    ...
    assert logging.getLogger("tth_claude").propagate is True
    assert not logging.getLogger("tth_claude").handlers
