...
from tth_grok import settings
from tth_grok.shared.logs import UtcFormatter
...
def test_split_logger_propagates_to_root_handlers() -> None:
    ...
    try:
        logging.getLogger("tth_grok.tests.propagation").info("hello from the split")
    ...
    assert logging.getLogger("tth_grok").propagate is True
    assert not logging.getLogger("tth_grok").handlers
