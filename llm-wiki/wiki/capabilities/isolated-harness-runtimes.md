...
Managed execution now binds to [project sandbox policies](../requirements/project-sandbox-policies.md).
Homes, caches, mounts, and networks are scoped by policy revision and provider.
Credential handles replace copied host secrets. Rebuild split and gateway images
before rollout. All seven installed provider versions pass the focused live
create/resume login gate. Forced refresh remains covered by deterministic tests.
