...
from tth_codex import settings
from tth_codex.shared.logs import UtcFormatter
...
def test_split_logger_propagates_to_root_handlers() -> None:
    ...
    try:
        logging.getLogger("tth_codex.tests.propagation").info("hello from the split")
    ...
    assert logging.getLogger("tth_codex").propagate is True
    assert not logging.getLogger("tth_codex").handlers
