...
from tth_codex import __version__ as split_version
from tth_codex.auth import SplitTokenAuth
from tth_codex.errors import register_exception_handlers
from tth_codex.service import KIND, create_session, probe
from tth_codex.sessions import get_session_store
from tth_codex.sse import stream_frames
...
async def terminate_session(request: HttpRequest, session_id: UUID) -> tuple[int, None]:
    if request.body:
        TerminateRequest.model_validate_json(request.body)
    store = get_session_store()
    store.get(session_id)
    # SDK-managed harness: no supervised process; terminate collapses to close.
    await store.close(session_id, reason="terminated")
    return 204, None
