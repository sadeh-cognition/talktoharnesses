...
from tth_prime_agent import settings
from tth_prime_agent.shared.logs import UtcFormatter
...
def test_split_logger_propagates_to_root_handlers() -> None:
    ...
    try:
        logging.getLogger("tth_prime_agent.tests.propagation").info("hello from the split")
    ...
    assert logging.getLogger("tth_prime_agent").propagate is True
    assert not logging.getLogger("tth_prime_agent").handlers
