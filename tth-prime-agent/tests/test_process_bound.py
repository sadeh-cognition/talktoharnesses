...
class SpawnFakeAdapter:
    ...
    async def probe(self, config: HarnessConfiguration) -> HarnessCapabilities:
        del config
        return HarnessCapabilities(kind=HarnessKind.PRIME_AGENT, version="1.0.3-R2198.1")
...
def spawn_adapter(monkeypatch: pytest.MonkeyPatch) -> Iterator[SpawnFakeAdapter]:
    ...
    monkeypatch.setenv("TALKTOHARNESSES_PRIME_AGENT_EXECUTABLE", sys.executable)
...
async def test_spawn_layers_spec_environment_over_the_parent_environment(
    ...
    launch = LaunchSnapshot(
        ...
        working_directory=str(tmp_path),
        adapter_version="test",
        capabilities=HarnessCapabilities(kind=HarnessKind.PRIME_AGENT, version="1"),
    )
    script = (
