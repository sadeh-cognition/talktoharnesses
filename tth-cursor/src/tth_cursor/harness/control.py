...
async def set_cursor_config_option(
    ...
    *,
    ...
    if option.currentValue == value:
        return options
    result: object = None
    for attempt in range(_SET_CONFIG_ATTEMPTS):
        try:
            future, _ = await connection.request(
            ...
            # Freshly launched CLIs intermittently answer -32603 (internal
            # error) for an advertised value; only that code is retryable.
            if exc.code != _JSONRPC_INTERNAL_ERROR or attempt == _SET_CONFIG_ATTEMPTS - 1:
                raise DomainError(
                    ErrorCode.PROVIDER_INCOMPATIBLE,
                    "Cursor rejected configuration option",
                    details={"config_id": config_id, "remote_code": exc.code},
                ) from exc
            await asyncio.sleep(_SET_CONFIG_RETRY_DELAY)
