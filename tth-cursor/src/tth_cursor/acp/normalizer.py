...
class AcpSessionNormalizer:
        ...
        return list(
            self._usage.replace(
                ...
                cached_input_tokens=_optional_int(
                    update.get("cachedReadTokens")
                    or update.get("cachedInputTokens")
                    or update.get("cached_input_tokens")
                ),
            )
        )
    ...
    def _normalize_approval_action(
        ...
        from tth_types.enums import FileOperation
        from tth_types.harness import (
            CommandApprovalAction,
            FileApprovalAction,
    ...
    def on_prompt_terminal(
        ...
        *,
        ...
        turn_id = self._require_turn()
        events: list[HarnessEvent] = []
        usage_map = _as_dict(usage)
        if usage_map is not None:
            events.extend(self._usage_update(usage_map))
