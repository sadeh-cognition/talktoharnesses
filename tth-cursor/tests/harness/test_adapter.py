    ...
    result = await adapter._on_update_todos_request(  # pyright: ignore[reportPrivateUsage]
        SimpleNamespace(
            ...
            params={
                ...
                "merge": False,
                "todos": [{"id": "a", "content": "Do the thing", "status": "in_progress"}],
            },
        )
    )
    assert result == {}
...
class _PendingPromptProcess(_FakeAcpProcess):
    """Never resolves session/prompt, keeping the prompt watcher live."""
...
def _prompt_calls(proc: _FakeAcpProcess) -> list[dict[str, Any]]:
    return [msg for msg in proc.requests if msg.get("method") == "session/prompt"]


@pytest.mark.asyncio
async def test_duplicate_submit_same_turn_is_noop_while_prompt_active() -> None:
    pending = _PendingPromptProcess(
        agent_name="cursor",
        agent_version="2026.08.04-aaa8809",
    )
    adapter, proc = await _probed_adapter(pending)
    session = await adapter.start(
        StartSessionRequest(
            conversation_id=uuid4(),
            binding_id=uuid4(),
    ...
    assert len(_prompt_calls(proc)) == 1
    await adapter.close(session)
