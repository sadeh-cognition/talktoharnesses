...
def test_plan_create_update_and_usage_optional_int() -> None:
    ...
    assert isinstance(terminal[0], UsageUpdatedPayload)
    assert terminal[0].input_tokens == 10
    assert terminal[0].output_tokens == 4
    assert terminal[0].total_tokens == 14
    assert terminal[0].cached_input_tokens == 3
    assert isinstance(terminal[-1], TurnCompletedPayload)
