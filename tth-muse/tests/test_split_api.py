...
async def test_probe(fake_adapter: FakeAdapter, tmp_path: Any) -> None:
    ...
    assert probe.capabilities.kind is HarnessKind.MUSE
...
async def test_busy_submit_maps_to_conflict(fake_adapter: FakeAdapter, tmp_path: Any) -> None:
    """A CONVERSATION_BUSY duplicate-turn rejection surfaces as HTTP 409, not
    a turn-killing failure."""
    ...
    async def busy_submit(session: Any, request: Any) -> None:
        raise DomainError(ErrorCode.CONVERSATION_BUSY, "turn already active")

    fake_adapter.submit = busy_submit  # type: ignore[method-assign]
    turn = TurnRequest(turn_id=uuid4(), prompt="dup")
    response = await _post(client, f"/v1/sessions/{created.session_id}/turns", turn)
    assert response.status_code == 409, response.content
    error = SplitError.model_validate_json(response.content)
    assert error.code == "conversation_busy"
...
async def test_capacity_refusal_closes_started_session(
    ...
    from tth_muse.sessions import get_session_store
