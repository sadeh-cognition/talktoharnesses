...
from tth_muse import settings
from tth_muse.shared.logs import UtcFormatter
...
def test_split_logger_propagates_to_root_handlers() -> None:
    ...
    try:
        logging.getLogger("tth_muse.tests.propagation").info("hello from the split")
    ...
    assert logging.getLogger("tth_muse").propagate is True
    assert not logging.getLogger("tth_muse").handlers
