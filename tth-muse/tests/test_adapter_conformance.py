"""Provider-neutral adapter contract suite over a fake MSP host."""
...
import pytest
from tests.fakes import capabilities_for, config_for, make_adapter_factory
...
from tth_muse import __version__

PROVIDER_KINDS = (HarnessKind.MUSE,)
...
async def test_probe_start_resume_submit_terminal_steer_interrupt_close_isolation(
    ...
    if caps.supports_steer:
        # The fake may complete before steer; False means orchestration queues.
    ...
    assert s_a.binding_id != s_b.binding_id
    await a.close(s_a)
    await b.close(s_b)
